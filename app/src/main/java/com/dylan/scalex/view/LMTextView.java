...
public class LMTextView extends TextView {
    private static final String TAG = LMTextView.class.getSimpleName();
    ...
    public LMTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void applyLetterSpacing() {
        ...
        if (textAllCaps) {
            builder = new StringBuilder(originalText.toString().toUpperCase().replaceAll("(.)", LMConstant.NON_BREAKING_SPACE + "$1"));
        } else {
            builder = new StringBuilder(originalText.toString().replaceAll("(.)", LMConstant.NON_BREAKING_SPACE + "$1"));
        }

        float nonBreakingSpace = getPaint().measureText(LMConstant.NON_BREAKING_SPACE);
